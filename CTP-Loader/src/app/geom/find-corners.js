...
(function (CTP) {
    ...
    (function (Geom) {
        ...
        //}
        function _findCorners(points, cornerCandidates, detail) {
            var cornerIndexes = [];
            // The minimum angle is the angle that can hide behind a seemingly straight line
            var p1 = new Geom.Point(0, 0);
            var p2 = new Geom.Point(detail, 1);
            var p3 = new Geom.Point(detail * 2, 0);
            ...
            for (var iP = 0, iC = 0; (iP < pLen) && (iC < cLen); iC++) {
                ...
                if (iSearch >= 0) {
                    if (distance < 2 * detail) {
                        ...
                        A = points[iSearch];
                        B = new Geom.Point((A.x + C.x) / 2, (A.y + C.y) / 2);
                    }
                }
                if (iSearch < 0) {
                ...
                if (iSearch < pLen) {
                    if (distance < 2 * detail) {
                        ...
                        E = points[iSearch];
                        D = new Geom.Point((E.x + C.x) / 2, (E.y + C.y) / 2);
                    }
                }
                if (iSearch >= pLen) {
                    // Not enough room to decide whether this is a corner
        ...
        Geom._findCorners = _findCorners;
        //
        // 
        // ============================================================================
        function findCorners(points, cornerCandidates, detail) {
            var candidate, iC, iP, iSearch, distance, A, B, C, D, E, a1, a2, a3, aMin, cornerIndexes = [];
            ...
            function findAngle(p0, p1, p2) {
                ...
                var a = Math.pow(p1.x - p0.x, 2) + Math.pow(p1.y - p0.y, 2), b = Math.pow(p1.x - p2.x, 2) + Math.pow(p1.y - p2.y, 2), c = Math.pow(p2.x - p0.x, 2) + Math.pow(p2.y - p0.y, 2);
            ...
            for (iP = 0, iC = 0; (iP < points.length) && (iC < cornerCandidates.length); iC++) {
                ...
                //A & B
                iSearch = iP - 1;
                while ((iSearch >= 0) && (distance = getDistance(C, points[iSearch])) < detail) {
                    iSearch--;
                }
                if (iSearch >= 0) {
                    if (distance < (2 * detail)) {
                        B = points[iSearch];
                        iSearch--;
                        while ((iSearch >= 0) && (getDistance(B, points[iSearch]) < detail)) {
                            iSearch--;
                        }
                        if (iSearch >= 0) {
                            A = points[iSearch];
                ...
                //D & E
                iSearch = iP + 1;
                while ((iSearch < points.length) && (distance = getDistance(C, points[iSearch])) < detail) {
                    iSearch++;
                }
                if (iSearch < points.length) {
                    if (distance < (2 * detail)) {
                        D = points[iSearch];
                        iSearch++;
                        while ((iSearch < points.length) && (getDistance(D, points[iSearch]) < detail)) {
                            iSearch++;
                        }
                        if (iSearch < points.length) {
                            E = points[iSearch];
        ...
        Geom.findCorners = findCorners;
    })(Geom = CTP.Geom || (CTP.Geom = {}));
})(CTP || (CTP = {}));
//# sourceMappingURL=find-corners.js.map