...
(function (CTP) {
    ...
    (function (routes) {
        ...
        (function (controlPanel) {
            ...
            var findCorners = CTP.Geom.findCorners;
            var simplify = CTP.Geom.simplify;
            var simplifyPath = CTP.Geom.simplifyPath;
            var StitchType = CTP.Loader.StitchType;
            ...
            var PathFinder = (function () {
                ...
                PathFinder.prototype.find = function (start, end, points) {
                    ...
                    var sx = start.x;
                    var sy = start.y;
                    points = _.reduce(points, function (res, point, i) {
                        var pn = new PathNode(null, point.x, point.y);
                        pn.d = Math.pow((sx - pn.x), 2) + Math.pow((sy - pn.y), 2);
                        res.push(pn);
                        return res;
                    }, []);
                    ...
                    var idx = 0;
                    var pts = _.takeWhile(points, function (value, i, array) {
                        idx++;
                    ...
                    while (this.open.length) {
                        //while (ii < len) {
                        var q = _.minBy(this.open, function (node) {
                        ...
                        if (p1) {
                            ...
                            var dg1 = Math.pow((q.x - n1.x), 2) + Math.pow((q.y - n1.y), 2);
                            var dh1 = Math.pow((end.x - n1.x), 2) + Math.pow((end.y - n1.y), 2);
                            n1.g = dg1;
                            //n1.g = q.g + dg1;
                            //n1.g = q.g + dg1 + (q.parent && q.parent.g);
                            n1.h = dh1;
                            n1.f = n1.g + n1.h;
                            var o1 = _.filter(this.open, function (node) {
                            ...
                            var c1 = _.filter(this.closed, function (node) {
                                return node.x === n1.x && node.y === n1.y && node.f <= n1.f;
                                //return node.f < n1.f;
                            })[0];
                            if (c1) {
            ...
            var PointSprite = (function (_super) {
                __extends(PointSprite, _super);
                ...
                PointSprite.prototype.setSelected = function () {
                ...
                PointSprite.prototype.onClick = function () {
                    this.setSelected();
                    //console.log("CLICKED POINT");
                };
                return PointSprite;
            }(PIXI.Sprite));
            var PatchLayer = (function () {
                function PatchLayer($scope, $element, $timeout) {
                    //console.log("\n\nLAYER SCOPE", $scope);
                    ...
                    // TEST
                    this.shapes = this.layer._shapes;
                    this.pointSprites = [];
                    this.matrix = [];
                    ...
                    this.matrix2 = [];
                    this.canvas = $element.find(".patch-layer-canvas")[0];
                ...
                PatchLayer.prototype.editShape = function (shape) {
                    this.patchDesign.editShape(shape, this);
                    this.renderShape(shape);
                };
                PatchLayer.prototype.updateShape = function () {
                    var shape = this.patchDesign.currentShape;
                    this.renderShape(shape);
                    //console.log("UPDATE SHAPE");
                };
                PatchLayer.prototype.generateGrid = function () {
                ...
                PatchLayer.prototype.runFinder = function (shape, start, end) {
                ...
                PatchLayer.prototype.runStar = function (shape, start, end) {
                    var w = +this.patchWidth;
                    var h = +this.patchHeight;
                    ...
                    var good = [];
                    var points = _.reduce(shape.points, function (res, point, i) {
                        var x = (x + 1) % w;
                        var y = i % h;
                        good.push(x * y);
                    ...
                    var path = this.easyStar.findPath(start.x >> 0, start.y >> 0, end.x >> 0, end.y >> 0, function (p) {
                ...
                PatchLayer.prototype.findPath = function (shape) {
                    var graphics = this.graphics;
                    var start = _.sample(shape.points);
                    var end = _.sample(shape.points);
                    ...
                    path = _.reduce(path, function (res, point) {
                    ...
                    var pp = _.reduce(path, function (res, point, i, array) {
                        var minPoint = shape.points[0];
                        var minDist = Math.pow((minPoint.x - point.x), 2) + Math.pow((minPoint.y - point.y), 2);
                        var prev = minPoint;
                        _.forEach(shape.points, function (pt) {
                            var d1 = Math.pow((pt.x - point.x), 2) + Math.pow((pt.y - point.y), 2);
                ...
                PatchLayer.prototype.alignPath = function (shape, path) {
                    var points = shape.points;
                    var prev = points[0];
                };
                PatchLayer.prototype.renderShape = function (shape) {
                    var _this = this;
                    var graphics = this.graphics;
                    this.graphics.clear();
                    _.forEach(this.pointSprites, function (sprite) {
                        _this.stage.removeChild(sprite);
                        sprite.destroy();
                    });
                    this.pointSprites = [];
                    ...
                    _.forEach(shape.simplifiedPoints, function (point) {
                        graphics.lineTo(point.x, point.y);
                    });
                    graphics.endFill();
                    this.render();
                    ...
                    _.forEach(shape.simplifiedPoints, function (point) {
                        ...
                        var sprite = new PointSprite(_this.circleTexture);
                ...
                PatchLayer.prototype.drawClipper = function () {
                    var graphics = this.graphics;
                ...
                PatchLayer.prototype.drawEnds = function (drawLine, alpha) {
                    ...
                    var green = 0x00cc00;
                    var lineWidth = 0;
                    _.forEach(shapes, function (shape, j) {
                        //console.log("\nCOUNT 1", j);
                        if (!shape.length) {
                            return true;
                        }
                        //console.log("COUNT 2", j);
                        _.forEach(shape, function (point, i, points) {
                ...
                PatchLayer.prototype.drawPSLG = function () {
                    var graphics = this.graphics;
                    //var cornerIndexes = this.layer.cornerIndexes;
                    var shapes = _.reduce(this.layer.p3, function (res, shape, i) {
                        var output = [];
                        ...
                        //    points.push(point);
                        //}
                        var shapePoints = _.reduce(shape.points, function (points, point, j) {
                    ...
                    _.forEach(shapes, function (shape, i) {
                        //var fill = 0x4CAF50;
                        var fill = 0xffd900;
                        fill = 0xff0000;
                        //fill = 0x990099;
                ...
                PatchLayer.prototype.drawShapes = function () {
                    var graphics = this.graphics;
                    ...
                    graphics.lineStyle(2, lineColor, 1);
                    _.forEach(this.layer.shapes, function (shape) {
                        _.forEach(shape, function (point, i) {
                            if (!i) {
                                graphics.endFill();
                                graphics.moveTo(point.x, point.y);
                            }
                ...
                PatchLayer.prototype.drawLayer = function () {
                    var _this = this;
                    ...
                    var next;
                    var moved = false;
                    _.forEach(this.layer.points, function (point, i, points) {
                        //_.forEach(this.layer.stitches, point => {
                        prev = points[i - 1];
                        next = points[i + 1];
                        var stitch = _this.layer.stitches[i];
                        var flag = _this.layer.stitches[i].flags;
                        //switch (stitch.flags) {
                ...
                PatchLayer.prototype.render = function () {
